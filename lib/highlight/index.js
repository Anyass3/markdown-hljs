(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.$3 = {}));
  ...
  */
  ...
  function sql(hljs) {
    ...
    const FUNCTION_CALL = {
      begin: concat$a(/\b/, either$2(...FUNCTIONS), /\s*\(/),
      keywords: {
  ...
  const keywordWrapper = keyword => concat$b(
    /\b/,
    keyword,
    /\w$/.test(keyword) ? /\b/ : /\B/
  );
  ...
  // Valid operator.
  const operator = concat$b(operatorHead, operatorCharacter, '*');
  ...
  // Valid identifier.
  const identifier = concat$b(identifierHead, identifierCharacter, '*');

  // Valid type identifier.
  const typeIdentifier = concat$b(/[A-Z]/, identifierCharacter, '*');

  // Built-in attributes, which are highlighted as keywords.
  // @available is handled separately.
  const keywordAttributes = [
    'autoclosure',
    concat$b(/convention\(/, either$3('swift', 'block', 'c'), /\)/),
    'discardableResult',
    'dynamicCallable',
    'dynamicMemberLookup',
    'escaping',
  ...
  */
  ...
  function swift(hljs) {
    ...
    const DOT_KEYWORD = {
      className: 'keyword',
      begin: concat$b(/\./, lookahead$3(either$3(...dotKeywords, ...optionalDotKeywords))),
      end: either$3(...dotKeywords, ...optionalDotKeywords),
      excludeBegin: true
    };
    const KEYWORD_GUARD = {
      // Consume .keyword to prevent highlighting properties and methods as keywords.
      begin: concat$b(/\./, either$3(...keywords)),
      relevance: 0
    };
    ...
    const KEYWORD = {
      variants: [
        {
          className: 'keyword',
          begin: either$3(...REGEX_KEYWORDS, ...optionalDotKeywords)
        }
      ]
    };
    ...
    const BUILT_IN_GUARD = {
      ...
      begin: concat$b(/\./, either$3(...builtIns)),
      relevance: 0
    };
    const BUILT_IN = {
      className: 'built_in',
      begin: concat$b(/\b/, either$3(...builtIns), /(?=\()/)
    };
    const BUILT_INS = [
      BUILT_IN_GUARD,
      BUILT_IN
    ...
    const ESCAPED_CHARACTER = (rawDelimiter = "") => ({
      ...
      variants: [
        {
          begin: concat$b(/\\/, rawDelimiter, /[0\\tnr"']/)
        },
        {
          begin: concat$b(/\\/, rawDelimiter, /u\{[0-9a-fA-F]{1,8}\}/)
        }
      ]
    });
    const ESCAPED_NEWLINE = (rawDelimiter = "") => ({
      className: 'subst',
      begin: concat$b(/\\/, rawDelimiter, /[\t ]*(?:[\r\n]|\r\n)/)
    });
    ...
    const KEYWORD_ATTRIBUTE = {
      className: 'keyword',
      begin: concat$b(/@/, either$3(...keywordAttributes))
    };
    ...
    const TYPE = {
      begin: lookahead$3(/\b[A-Z]/),
      relevance: 0,
      contains: [
        { // Common Apple frameworks, for relevance boost
          className: 'type',
          begin: concat$b(/(AV|CA|CF|CG|CI|CL|CM|CN|CT|MK|MP|MTK|MTL|NS|SCN|SK|UI|WK|XC)/, identifierCharacter, '+')
        },
        { // Type identifier
          className: 'type',
        ...
        { // Protocol composition
          begin: concat$b(/\s+&\s+/, lookahead$3(typeIdentifier)),
          relevance: 0
        }
      ]
    };
  ...
  function ruby(hljs) {
    ...
    var RUBY_DEFAULT_CONTAINS = [
      ...
      },
      {
        ...
        begin: concat$c(/def\s*/, lookahead$4(RUBY_METHOD_RE + "\\s*(\\(|;|$)")),
        keywords: "def",
        end: '$|;',
        contains: [
  ...
  */
  ...
  async function setLang({ lang = '', alias = [], fn }) {
  ...
  const getlang = (lang) => {
      for (let language of languages) {
          if (language[0] === lang || language.alias.includes(lang))
              return language[0];
      }
      return lang;
  };
  ...
  const Highlight = (source) => {
      ...
      while (true) {
          ...
          if (prev_code === code && prev_lang === lang)
              break;
          prev_code = code;
          prev_lang = lang;
          const highlightedCode = highlightCode(lang, code);
          if (lang)
              source = source.replace(result[0], `<pre><code class='hljs ${lang}'>${highlightedCode}</pre></code>`);
          else
  ...
  exports.getHljs = getHljs;
  exports.getlang = getlang;
  exports.highlightCode = highlightCode;
  exports.setHljs = setHljs;
